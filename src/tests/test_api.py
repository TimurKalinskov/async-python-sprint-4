...
async def test_get_create(client: AsyncClient) -> None:
    ...
    response = await client.get('api/v1/url_shorts/1')
...
async def test_status(client: AsyncClient) -> None:
    ...
    response = await client.get('api/v1/url_shorts/1/status')
    assert response.status_code == status.HTTP_200_OK
    ...
    response = await client.get(
        'api/v1/url_shorts/1/status', params={'full_info': True}
    )
    assert response.status_code == status.HTTP_200_OK
...
async def test_delete(client: AsyncClient) -> None:
    ...
    response = await client.delete('api/v1/url_shorts/1')
